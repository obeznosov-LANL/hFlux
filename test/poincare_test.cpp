...
void run(const int nR_data, const int nZ_data) {
  ...
  const int n_turn = 1000;
  const int n_traces = n_r * n_theta;

  // poincare_data(q, trace, turn): q = 0 -> R, q = 1 -> Z.
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int ir, const int itheta) {
        pd_d(0, ir + itheta * n_r, 0) =
            R_center + ir * dr * Kokkos::cos(itheta * dtheta);
        pd_d(1, ir + itheta * n_r, 0) =
            Z_center + ir * dr * Kokkos::sin(itheta * dtheta);
      });
  ...
  // Dump the Poincare section (all turns) to a text file for inspection.
  poincare_data.sync_host();
  auto pd_h = poincare_data.view_host();
  std::FILE* out = std::fopen("poincare_test.txt", "w");
  if (out != nullptr) {
